...
int dwm_pos_set(dwm1001_ctx_t * device, dwm_pos_t* pos)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   ...
   *(uint32_t*)(tx_data+tx_len) = pos->z;
   ...
   device->writeData(device->handle, tx_data, tx_len);      
   return device->readData(device->handle, rx_data, &rx_len, 3);
}

int dwm_pos_get(dwm1001_ctx_t * device, dwm_pos_t* p_pos)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   uint8_t data_cnt;
   ...
   tx_data[tx_len++] = 0;   
   device->writeData(device->handle, tx_data, tx_len);   
   if(device->readData(device->handle, rx_data, &rx_len, 18) == RV_OK)
   {
...
int dwm_upd_rate_set(dwm1001_ctx_t * device, uint16_t ur, uint16_t ur_static)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_upd_rate_get(dwm1001_ctx_t * device, uint16_t *ur, uint16_t *ur_static)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   uint8_t data_cnt;
   ...
   tx_data[tx_len++] = 0;   
   device->writeData(device->handle, tx_data, tx_len);   
   if(device->readData(device->handle, rx_data, &rx_len, 9) == RV_OK)
   {
      data_cnt = RESP_DAT_VALUE_OFFSET;
...
int dwm_cfg_tag_set(dwm1001_ctx_t * device, dwm_cfg_tag_t* cfg) 
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_cfg_anchor_set(dwm1001_ctx_t * device, dwm_cfg_anchor_t* cfg)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_cfg_get(dwm1001_ctx_t * device, dwm_cfg_t* cfg)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   ...
   tx_data[tx_len++] = 0;   
   device->writeData(device->handle, tx_data, tx_len);   
   if(device->readData(device->handle, rx_data, &rx_len, 7) == RV_OK)
   {
...
int dwm_sleep(dwm1001_ctx_t * device)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_anchor_list_get(dwm1001_ctx_t * device, dwm_anchor_list_t *p_list)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   uint8_t data_cnt, i;
   ...
   tx_data[tx_len++] = 0;   
   device->writeData(device->handle, tx_data, tx_len);   
   if(device->readData(device->handle, rx_data, &rx_len, DWM1001_TLV_MAX_SIZE) == RV_OK)
   {
...
int dwm_loc_get(dwm1001_ctx_t * device, dwm_loc_data_t* loc)
{ 
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_baddr_set(dwm1001_ctx_t * device, dwm_baddr_t* p_baddr)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_baddr_get(dwm1001_ctx_t * device, dwm_baddr_t* p_baddr)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   uint8_t data_cnt;
   ...
   tx_data[tx_len++] = 0;   
   device->writeData(device->handle, tx_data, tx_len);   
   if(device->readData(device->handle, rx_data, &rx_len, 11) == RV_OK)
   {
      data_cnt = RESP_DAT_VALUE_OFFSET;
...
int dwm_stnry_cfg_set(dwm1001_ctx_t * device, dwm_stnry_sensitivity_t sensitivity)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_stnry_cfg_get(dwm1001_ctx_t * device, dwm_stnry_sensitivity_t* p_sensitivity)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   ...
   tx_data[tx_len++] = 0;   
   device->writeData(device->handle, tx_data, tx_len);   
   if(device->readData(device->handle, rx_data, &rx_len, 6) == RV_OK)
   {
...
int dwm_factory_reset(dwm1001_ctx_t * device)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_reset(dwm1001_ctx_t * device)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_ver_get(dwm1001_ctx_t * device, dwm_ver_t* ver)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   uint8_t data_cnt;
   ...
   tx_data[tx_len++] = 0;   
   device->writeData(device->handle, tx_data, tx_len);   
   if(device->readData(device->handle, rx_data, &rx_len, 21) == RV_OK)
   {    
      // read fw_version
...
int dwm_uwb_cfg_set(dwm1001_ctx_t * device, dwm_uwb_cfg_t* p_cfg) 
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_uwb_cfg_get(dwm1001_ctx_t * device, dwm_uwb_cfg_t* p_cfg) 
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   uint8_t data_cnt;
   ...
   tx_data[tx_len++] = 0;   
   device->writeData(device->handle, tx_data, tx_len);   
   if(device->readData(device->handle, rx_data, &rx_len, 15) == RV_OK)
   {
      data_cnt = RESP_DAT_VALUE_OFFSET;
...
int dwm_usr_data_read(dwm1001_ctx_t * device, uint8_t* p_data, uint8_t* p_len)
{     
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_usr_data_write(dwm1001_ctx_t * device, uint8_t* p_data, uint8_t len, bool overwrite)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_label_read(dwm1001_ctx_t * device, uint8_t* p_label, uint8_t* p_len)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_label_write(dwm1001_ctx_t * device, uint8_t* p_label, uint8_t len)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_gpio_cfg_output(dwm1001_ctx_t * device, dwm_gpio_idx_t idx, bool value)
{      
   if(dwm_check_gpio_idx(idx)!= RV_OK)
   {
      return RV_ERR;
   }
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
...
int dwm_gpio_cfg_input(dwm1001_ctx_t * device, dwm_gpio_idx_t idx, dwm_gpio_pin_pull_t pull_mode)
{      
   if(dwm_check_gpio_idx(idx)!= RV_OK)
   {
      return RV_ERR;
...
int dwm_gpio_value_set(dwm1001_ctx_t * device, dwm_gpio_idx_t idx, bool value)
{
   if(dwm_check_gpio_idx(idx)!= RV_OK)
   {
      return RV_ERR;
...
int dwm_gpio_value_get(dwm1001_ctx_t * device, dwm_gpio_idx_t idx, bool* p_value)
{
   if(dwm_check_gpio_idx(idx)!= RV_OK)
   {
      return RV_ERR;
...
int dwm_gpio_value_toggle(dwm1001_ctx_t * device, dwm_gpio_idx_t idx)
{
   if(dwm_check_gpio_idx(idx)!= RV_OK)
   {
      return RV_ERR;
...
int dwm_panid_set(dwm1001_ctx_t * device, uint16_t value)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_panid_get(dwm1001_ctx_t * device, uint16_t* p_value)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_node_id_get(dwm1001_ctx_t * device, uint64_t *p_node_id)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   uint8_t data_cnt;
   ...
   tx_data[tx_len++] = 0; 
   device->writeData(device->handle, tx_data, tx_len);  
   if(device->readData(device->handle, rx_data, &rx_len, 13) == RV_OK)
   {
      data_cnt = RESP_DAT_VALUE_OFFSET;
...
int dwm_status_get(dwm1001_ctx_t * device, dwm_status_t* p_status)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   uint16_t flags;
...
int dwm_int_cfg_set(dwm1001_ctx_t * device, uint16_t value)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_int_cfg_get(dwm1001_ctx_t * device, uint16_t *p_value)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_bh_status_get(dwm1001_ctx_t * device, bh_status_t * p_bh_status)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   uint8_t data_cnt, i;
...
int dwm_enc_key_set(dwm1001_ctx_t * device, dwm_enc_key_t* p_key)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   uint8_t i;
...
int dwm_enc_key_clear(dwm1001_ctx_t * device)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_uwb_preamble_code_set(dwm1001_ctx_t * device, dwm_uwb_preamble_code_t code)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_uwb_preamble_code_get(dwm1001_ctx_t * device, dwm_uwb_preamble_code_t *p_code)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_uwb_scan_start(dwm1001_ctx_t * device)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
...
int dwm_uwb_scan_result_get(dwm1001_ctx_t * device, dwm_uwb_scan_result_t *p_result)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE], rx_len;
   uint8_t data_cnt, i;
...
int dwm_check_gpio_idx(dwm_gpio_idx_t idx)
{
